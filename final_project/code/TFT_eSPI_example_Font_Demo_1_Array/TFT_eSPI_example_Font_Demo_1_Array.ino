...
{"A1 7C 8D 1D", "Family shotglass", "June 6, 2020", "Wuhan, China", "No", "Hello! This is a fun note."},
 ...
 /* DOWN BUTTON*/
  ...
  if (screen == 1) {
    ...
    if (shotglass == "") {
      ...
      tft.print("ID Number: ");
      tft.println(shots[0][0]);
      tft.print("Name: ");
      tft.println(shots[0][1]);
      tft.print("Date Acquired: ");
      tft.println(shots[0][2]);
      tft.print("Location Acquired: ");
      tft.println(shots[0][3]);
    }
    else {
      tft.println("Long lines wrap to the next line");
      tft.print("ID Number: ");
...
//    delay(1000);
    ...
    tft.println("Rainbow = A Favorite Shotglass");
  }
  else {
    tft.setTextColor(TFT_WHITE, TFT_BLACK); // Set the font colour AND the background colour
    tft.setCursor(0, 0); // Set cursor at top left of screen
